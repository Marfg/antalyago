// Prototype-only rendering bridge: motion effects share the production camera.
 ...
 function draw(now){
  ...
  if(scenario&&effect){
   ...
   if(t===1)effect=null;
    ...
    for(const stone of effect.captures){const p=project(stone,true);ctx.fillStyle=stone.color==='black'?'#171923':'#efece3';ctx.strokeStyle=stone.color==='black'?'#555967':'#b4b0a6';ctx.lineWidth=1;ctx.beginPath();ctx.ellipse(p.x,p.y-t*15,p.rx*(1-t*.15),p.ry*(1-t*.15),0,0,Math.PI*2);ctx.fill();ctx.stroke()}
