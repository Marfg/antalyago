...
const adapter={setSize(size){assert.equal(size,9)},reset(){visualBoard=new BoardState(CONNECTION_BOARD_SIZE)},clearMovePreview(){preview=null},clearLiberties(){},clearIllegalHints(){},clearRegionMarks(){},focus(name){assert.equal(name,'connections')},focusPoints(){},showLiberties(){},setInputEnabled(){},onIntersectionTap(fn){tap=fn;return ()=>{tap=null}},onIntersectionHover(fn){hover=fn;return ()=>{hover=null}},setMovePreview(p){preview=p},playMove(p){if(!isValidMove(visualBoard,p.col,p.row,p.color).valid)return {ok:false};visualBoard=applyMove(visualBoard,p.col,p.row,p.color).newState;return {ok:true}}};
