...
/* ══════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════ */
// Native capture coverage replaces the curriculum-specific I/J/K quiz cases.
addTest('Capture native: desktop/mobile drills and first-capture game', async () => {
  const s = await openScenesPage({ query: PREVIEW_QUERY });
  try {
    await s.page.goto(BASE + '/tests/captureNative.browser.html');
    await s.page.waitForFunction(() => document.body.dataset.result, null, { timeout: 120000 });
    ensure(await s.page.locator('body').getAttribute('data-result') === 'passed', await s.page.locator('#results').textContent());
  } finally { await s.close(); }
});
// LS-native coverage replaces the old curriculum-specific scene 8 assertions.
addTest('Illegal moves native: 9x9 experiments, transfer and lifecycle', async () => {
  const s = await openScenesPage({ query: PREVIEW_QUERY });
  try {
    await s.page.goto(BASE + '/tests/illegalNative.browser.html');
    await s.page.waitForFunction(() => document.body.dataset.result, null, { timeout: 120000 });
    ensure(await s.page.locator('body').getAttribute('data-result') === 'passed', await s.page.locator('#results').textContent());
  } finally { await s.close(); }
});
async function completeScene8(page) {
  await page.goto(BASE + '/' + PAGE + PREVIEW_QUERY + '&testScene=' + S08_ID);
  await page.waitForSelector('.illegal-native');
  const { STAGES } = await import('../scenes/illegalNativePolicy.js');
  for (const stage of STAGES) {
    await page.waitForTimeout(800);
    for (const target of stage.targets) {
      const pt = await findScreenPointFor(page, target);
      ensure(pt, 'Scene 8 target is outside the board viewport');
      await page.mouse.click(pt.x, pt.y);
      await page.waitForSelector('.illegal-native[data-phase="concept"]');
    }
    await page.click('.illegal-native [data-action="next"]');
  }
  await page.waitForSelector('.ls-topic-end [data-action="advance"]');
}
...
addTest('M1) Sahne #8 → Sahne #9 GERÇEK topic-end geçişi: Sahne #8 artık son sahne DEĞİL, anlatım alanı sahneye uyarlanır, tek scene_started', async () => {
  const s = await openScenesPage({ query: PREVIEW_QUERY });
  try {
    ...
    const boardAfter = await s.page.locator('#ls-canvas').boundingBox();
    const narrAfter = await s.page.locator('#ls-narration').boundingBox();
    ensure(Math.abs(boardBefore.width - boardAfter.width) < 1 && boardAfter.height > 0, 'geçişte tahta genişliği veya görünürlüğü bozuldu');
...
addTest('N1) Sahne #9 → Sahne #10 GERÇEK topic-end geçişi: Sahne #9 artık son sahne DEĞİL, anlatım alanı sahneye uyarlanır, tek scene_started', async () => {
  const s = await openScenesPage({ query: PREVIEW_QUERY });
  try {
